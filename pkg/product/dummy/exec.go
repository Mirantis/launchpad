...
func (p *Dummy) Exec(target string, interactive, first bool, role, cmd string) error {
	// This stuff is gpretty generic for any Connectable
	var host *api.Host
	if target == "" {
		if role != "" {
			return fmt.Errorf("dummy has no roles")
		} else if first {
			...
			return fmt.Errorf("--target or --first required") // feels like this is in the wrong place
		}
