...
func (p *MKE) Exec(targets []string, interactive, first, all, parallel bool, role, hostos, cmd string) error { //nolint:maintidx
	...
	for _, target := range targets {
		...
		case strings.Contains(target, ":"):
			...
			host := p.ClusterConfig.Spec.Hosts.Find(func(h *mkeconfig.Host) bool {
				if h.Address() != addr {
					return false
				}
				if h.WinRM != nil {
	...
	if role != "" {
		if len(hosts) == 0 {
			hosts = p.ClusterConfig.Spec.Hosts.Filter(func(h *mkeconfig.Host) bool { return h.Role == role })
		} else {
			hosts = hosts.Filter(func(h *mkeconfig.Host) bool { return h.Role == role })
		}
	}
	...
	if hostos != "" {
		...
		var foundhosts mkeconfig.Hosts
		...
		err := hosts.ParallelEach(func(host *mkeconfig.Host) error {
			...
			if err := host.ResolveConfigurer(); err != nil {
				return fmt.Errorf("failed to resolve configurer for host %s: %w", host.Address(), err)
			}
			if host.IsWindows() {
				if hostos == "windows" {
					mutex.Lock()
	...
	err = hosts.Each(func(h *mkeconfig.Host) error {
		if h.IsWindows() {
			if linuxcount > 0 {
